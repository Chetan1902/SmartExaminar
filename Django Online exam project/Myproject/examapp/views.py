...
def endexam(request):
    if 'answer' in request.session:
        ...
        auth.logout(request) 
        return render(request,'app1/results.html',{'correct_wrong': correct_and_wronganslist,'feedback':feedback,'score':score,'wrong':wrong,'username':username,'percentage':percentage,'total_question':totalquestion,"profile_pic":profile_pic})
